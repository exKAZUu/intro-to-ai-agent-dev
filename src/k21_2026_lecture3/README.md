...
4. 外部情報取得をツール化する
   - `example06.ts`: Tavily検索を自作ツールとして定義します。
   - `example07.ts`: OpenAI Hosted web search を使います。
5. 表形式データと構造化結果を扱う
   - `example08.ts`: Hosted code interpreter で `survey.csv` を分析します。
   - `example09.ts`: 集計ツールと Structured output を組み合わせます。
6. エージェントらしい制御に広げる
   - `example10.ts`: Handoff で分析担当と改善計画担当に分けます。

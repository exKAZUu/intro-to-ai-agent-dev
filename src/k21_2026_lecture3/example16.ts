...
最新情報は web_search で確認し、アンケート集計は compute_survey_stats、授業時間配分は compute_rotation_plan を使ってください。
selectedExamples は tools、hosted tools、structured output、handoffs、guardrails、tracing、MCP の中から、次回90分授業で扱う3つを選んでください。
最終出力は指定された構造に従ってください。
`.trim(),
  model: 'gpt-5-mini',
  tools: [webSearchTool({ searchContextSize: 'low' }), computeSurveyStats, computeRotationPlan],

...
type WorkshopImprovementReport = z.infer<typeof WorkshopImprovementReport>;

const accessLog = {
  cacheHitRate: 0.72,
  totalRequests: 8_987_654_321_234_567,
  weeklyRegularPracticePageRequests: 87_654_321_987,
  weeklySupplementPracticePageRequests: 12_345_678_901,
  weeks: 89,
};
...
学習サイト利用ログ:
- 総リクエスト数: ${accessLog.totalRequests.toLocaleString('en-US')}
- 通常演習ページの週次アクセス数: ${accessLog.weeklyRegularPracticePageRequests.toLocaleString('en-US')}
- 補講演習ページの週次アクセス数: ${accessLog.weeklySupplementPracticePageRequests.toLocaleString('en-US')}
- 対象期間: ${accessLog.weeks}週間
- キャッシュヒット率: ${accessLog.cacheHitRate * 100}%
...
await withTrace('workshop_integrated_workflow', async () => {
  ...
  const report = normalizeReport(response.finalOutput);
  console.log('\n=== 構造化された改善レポート ===\n');
...
function normalizeReport(report: WorkshopImprovementReport | undefined) {
  ...
  return {
    ...report,
    accessLogSummary: computeExpectedAccessLogSummary(),
  };
}

function computeExpectedAccessLogSummary() {
  const practicePageRequests =
    (accessLog.weeklyRegularPracticePageRequests + accessLog.weeklySupplementPracticePageRequests) * accessLog.weeks;
  const cacheHits = Math.round(accessLog.totalRequests * accessLog.cacheHitRate);
  return {
    practicePageRequests,
    otherRequests: accessLog.totalRequests - practicePageRequests,
    originRequests: accessLog.totalRequests - cacheHits,
  };
}
